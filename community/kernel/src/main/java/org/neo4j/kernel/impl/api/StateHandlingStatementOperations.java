        ...
        CountsOperations, LegacyIndexReadOperations, LegacyIndexWriteOperations
{
            ...
            throws IndexNotFoundKernelException, IndexBrokenKernelException, IndexNotApplicableKernelException
    {
        ...
        PrimitiveLongIterator changesFiltered =
                filterIndexStateChangesForSeek( statement, exactMatches, index, IndexQuery.asValueTuple( query ) );
            ...
            IndexQuery... predicates ) throws IndexNotFoundKernelException, IndexNotApplicableKernelException
    {
        ...
        case exact:
            ...
            return filterIndexStateChangesForSeek( statement, exactMatches, index, IndexQuery.asValueTuple( exactPreds ) );
        case stringSuffix:
        case stringContains:
        case exists:
    ...
    private PrimitiveLongIterator filterIndexStateChangesForSeek( KernelStatement state, PrimitiveLongIterator nodeIds,
            IndexDescriptor index, ValueTuple propertyValues )
    {
            ...
            throws EntityNotFoundException, InvalidTransactionTypeKernelException, AutoIndexingKernelException
    {
        ...
        try ( Cursor<NodeItem> cursor = nodeCursorById( state, nodeId ) )
        {
            ...
            {
                state.writableTxState().nodeDoAddProperty( node.id(), propertyKeyId, value );
            ...
            else
            {
                ...
                {
                    state.writableTxState().nodeDoChangeProperty( node.id(), propertyKeyId, existingValue, value );
            ...
            throws EntityNotFoundException, InvalidTransactionTypeKernelException, AutoIndexingKernelException
    {
        ...
        try ( Cursor<RelationshipItem> cursor = relationshipCursorById( state, relationshipId ) )
        {
            ...
            if ( !value.equals( existingValue ) )
            {
                state.writableTxState().relationshipDoReplaceProperty(
    ...
    public Value graphSetProperty( KernelStatement state, int propertyKeyId, Value value )
    {
        ...
        if ( !value.equals( existingValue ) )
        {
            state.writableTxState().graphDoReplaceProperty( propertyKeyId, existingValue, value );
        }
        ...
        try ( Cursor<NodeItem> cursor = nodeCursorById( state, nodeId ) )
        {
            ...
            try ( Cursor<PropertyItem> properties = nodeGetPropertyCursor( state, node, propertyKeyId ) )
            {
                if ( properties.next() )
                {
                    ...
                    state.writableTxState().nodeDoRemoveProperty( node.id(), propertyKeyId, existingValue );
    ...
    public Value graphRemoveProperty( KernelStatement state, int propertyKeyId )
    {
        ...
        if ( existingValue != Values.NO_VALUE )
        {
            state.writableTxState().graphDoRemoveProperty( propertyKeyId, existingValue );
        }
        return existingValue;
    }
